...
int log_impl_t::do_logfatal(const char* module_, const char* format_, ...)
{
    if (!m_opened)
    {
        cerr << "the log service have not open" << endl;
        return -1;
    }

    va_list app;
    va_start(app, format_);
    int ret = log_i(module_, LF_FATAL, format_, app, "", "");
    va_end(app);
...
int log_impl_t::do_logerror(const char* module_, const char* format_, ...)
{
    if (!m_opened)
    {
        cerr << "the log service have not open" << endl;
    ...
    va_list app;
    va_start(app, format_);
    int ret = log_i(module_, LF_ERROR, format_, app, "", "");
    va_end(app);
...
int log_impl_t::do_logwarn(const char* module_, const char* format_, ...)
{
    if (!m_opened)
    {
        cerr << "the log service have not open" << endl;
    ...
    va_list app;
    va_start(app, format_);
    int ret = log_i(module_, LF_WARN, format_, app, "", "");
    va_end(app);
...
int log_impl_t::do_loginfo(const char* module_, const char* format_, ...)
{
    if (!m_opened)
    {
        cerr << "the log service have not open" << endl;
    ...
    va_list app;
    va_start(app, format_);
    int ret = log_i(module_, LF_INFO, format_, app, "", "");
    va_end(app);
...
int log_impl_t::do_logdebug(const char* module_, const char* format_, ...)
{
    if (!m_opened)
    {
        cerr << "the log service have not open" << endl;
    ...
    va_list app;
    va_start(app, format_);
    int ret = log_i(module_, LF_DEBUG, format_, app, "", "");
    va_end(app);
                      ...
                      va_list va_list_, const char* color_begin_, const char* color_end_)
{
    ...
    if (ret)
    {
        cout << "log_impl_t::log_i check module err!"<< module_ << endl;
        return ret;
    }
    //! check flag
    ...
    vsnprintf(buff + length, m_max_line_size - length - 2, format_, va_list_);
    strcat(buff, "\n");
...
void log_impl_t::format_log_head_i(char* desc_, const char* module_, log_level_flag_e flag_)
{
    ...
    sprintf(desc_, "%04d%02d%02d %02d:%02d:%02d.%06ld %s [%s] : ", time->tm_year + 1900,
